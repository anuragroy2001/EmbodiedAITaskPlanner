...
""" + json.dumps(topology, indent=2)
    ...
    @staticmethod
    def generate_birds_eye_image_from_layout(layout_text: str):
        """Generate birds-eye map image from precomputed layout text only (no API for layout)."""
        if not client:
            raise ValueError("GenAI client not initialized.")
...
Layout details: {layout_text}"""
    ...
    def generate_birds_eye_view(gemini_images, topology):
        ...
        return VLAService.generate_birds_eye_image_from_layout(layout_text)
